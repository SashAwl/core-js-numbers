...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosAngle = (x1 * x2 + y1 * y2) / div;
...
function isPowerOfTwo(num) {
  ...
  return num % 2 === 0 ? isPowerOfTwo(Math.trunc(num / 2)) : false;
}
...
function isNumber(number) {
  const convertValue = parseFloat(number);
  return !Number.isNaN(convertValue) && Number.isFinite(number);
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.round((x1 + x2 + x3 + Number.EPSILON) * 1e10) / 1e10;
}
...
function getCountOfOddNumbers(number) {
  ...
  return Math.trunc(absNumber / 2) + (absNumber % 2 === 0 ? 0 : 1);
}
